...
@interface HSelectionList () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
...
const CGFloat kHSelectionListTrimHeight = 0.5;
const CGFloat kHSelectionListLabelCellInternalPadding = 15;

static NSString *LabelCellIdentifier = @"LabelCell";
static NSString *ViewCellIdentifier = @"ViewCell";

@implementation HSelectionList
...
- (void)commonInit {
    ...
    [_collectionView registerClass:[HSelectionListLabelCell class] forCellWithReuseIdentifier:LabelCellIdentifier];
    [_collectionView registerClass:[HSelectionListCustomViewCell class] forCellWithReuseIdentifier:ViewCellIdentifier];
    ...
    [_edgeFadeGradientView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(self);
        make.top.equalTo(self);
        make.bottom.equalTo(self).mas_offset(kHSelectionListTrimHeight);
    ...
    [_bottomTrim mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(self);
        make.height.mas_equalTo(kHSelectionListTrimHeight);
    }];


    _buttonInsets = UIEdgeInsetsMake(0, 5, 0, 5);
    _selectionIndicatorHeight = 3;
    _selectionIndicatorHorizontalPadding = kHSelectionListLabelCellInternalPadding/2;
    _selectionIndicatorStyle = HSelectionIndicatorStyleBottomBar;
    _selectionIndicatorAnimationMode = HSelectionIndicatorAnimationModeHeavyBounce;
...
- (void)reloadData {
    ...
    if (self.selectionIndicatorStyle == HSelectionIndicatorStyleBottomBar) {
    ...
    if (totalButtons > 0 && self.selectedButtonIndex >= 0 && self.selectedButtonIndex < totalButtons) {
        ...
        [((id<HSelectionListCell>)cell) setSelected:YES];
        
        switch (self.selectionIndicatorStyle) {
            case HSelectionIndicatorStyleBottomBar: {
                [self alignSelectionIndicatorWithCellAtIndexPath:selectedIndexPath];
                break;
            }

            case HSelectionIndicatorStyleButtonBorder: {
                if ([self.delegate respondsToSelector:@selector(selectionList:viewForItemWithIndex:)]) {
                    ((HSelectionListCustomViewCell *)cell).customView.layer.borderColor = self.selectionIndicatorColor.CGColor;
                } else {
...
- (void)setSelectedButtonIndex:(NSInteger)selectedButtonIndex animated:(BOOL)animated {
    ...
    NSInteger nextSelectedIndex = selectedButtonIndex;
    if(selectedButtonIndex < oldSelectedIndex){
        nextSelectedIndex = selectedButtonIndex -2;
    }else if(selectedButtonIndex > oldSelectedIndex){
        nextSelectedIndex = selectedButtonIndex +2;
    }
   
    if(nextSelectedIndex >= buttonCount){
        nextSelectedIndex = buttonCount -1;
    ...
    NSIndexPath *nextIndexPath = [NSIndexPath indexPathForItem:nextSelectedIndex inSection:0];
    
    UICollectionViewLayoutAttributes *selectedCellAttributes = [self.collectionView.collectionViewLayout layoutAttributesForItemAtIndexPath:nextIndexPath];
         ...
         usingSpringWithDamping:[self selectionIndicatorBarSpringDamping]
          initialSpringVelocity:0
                     ...
                     animations:^{
                         ...
                         [((id<HSelectionListCell>)selectedCell) setSelected:YES];
                         if(oldSelectedCell){
                             [((id<HSelectionListCell>)oldSelectedCell) setSelected:NO];
                         }
                         ...
                         switch (self.selectionIndicatorStyle) {
                             ...
                             case HSelectionIndicatorStyleButtonBorder: {
                                 if ([self.delegate respondsToSelector:@selector(selectionList:viewForItemWithIndex:)]) {
                                     ((HSelectionListCustomViewCell *)selectedCell).customView.layer.borderColor = self.selectionIndicatorColor.CGColor;

                                     if (oldSelectedCell != selectedCell) {
                                         ((HSelectionListCustomViewCell *)oldSelectedCell).customView.layer.borderColor = [UIColor clearColor].CGColor;
                                     }
                                 } else {
                             ...
                             case HSelectionIndicatorStyleNone: {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.dataSource respondsToSelector:@selector(selectionList:viewForItemWithIndex:)]) {
        ...
        [((HSelectionListCustomViewCell *)cell) setCustomView:[self.dataSource selectionList:self viewForItemWithIndex:indexPath.item]
        ...
        ((HSelectionListLabelCell *)cell).title = [self.dataSource selectionList:self titleForItemWithIndex:indexPath.item];
       
        for (NSNumber *controlState in [self.titleColorsByState allKeys]) {
            [((HSelectionListLabelCell *)cell) setTitleColor:self.titleColorsByState[controlState]
                                                               forState:controlState.integerValue];
        }

        for (NSNumber *controlState in [self.titleFontsByState allKeys]) {
            [((HSelectionListLabelCell *)cell) setTitleFont:self.titleFontsByState[controlState]
                                                              forState:controlState.integerValue];
        }
    }
    ...
    if ([self.dataSource respondsToSelector:@selector(selectionList:badgeValueForItemWithIndex:)]) {
        ...
        ((HSelectionListLabelCell *)cell).badgeValue = badgeValue;
    }

    if (self.selectionIndicatorStyle == HSelectionIndicatorStyleButtonBorder) {
        if ([self.delegate respondsToSelector:@selector(selectionList:viewForItemWithIndex:)]) {
            ((HSelectionListCustomViewCell *)cell).customView.layer.borderWidth = 1.0;
            ((HSelectionListCustomViewCell *)cell).customView.layer.cornerRadius = 3.0;
            ((HSelectionListCustomViewCell *)cell).customView.layer.masksToBounds = YES;
        } else {
...
- (CGSize)collectionView:(UICollectionView *)collectionView
  ...
  sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.dataSource respondsToSelector:@selector(selectionList:viewForItemWithIndex:)]) {
        ...
        CGSize titleSize = [HSelectionListLabelCell sizeForTitle:title withFont:self.titleFontsByState[@(UIControlStateNormal)]];

        CGFloat width = titleSize.width + horizontalPadding + kHSelectionListLabelCellInternalPadding;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.item == self.selectedButtonIndex) {
        if (self.selectionIndicatorStyle == HSelectionIndicatorStyleNone) {
...
- (void)alignSelectionIndicatorWithCellAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath) {
        ...
        self.selectionIndicatorBar.frame = CGRectMake(cellRect.origin.x + self.buttonInsets.left + kHSelectionListLabelCellInternalPadding/2 - self.selectionIndicatorHorizontalPadding,
                                                      self.contentView.frame.size.height - self.selectionIndicatorHeight,
                                                      cellRect.size.width - self.buttonInsets.left - self.buttonInsets.right - kHSelectionListLabelCellInternalPadding + 2*self.selectionIndicatorHorizontalPadding,
                                                      self.selectionIndicatorHeight);
    }
}
...
- (CGFloat)selectionIndicatorBarSpringDamping {
    switch (self.selectionIndicatorAnimationMode) {
        case HSelectionIndicatorAnimationModeHeavyBounce:
        default:
            return 0.5;

        case HSelectionIndicatorAnimationModeLightBounce:
            return 0.8;

        case HSelectionIndicatorAnimationModeNoBounce:
            return 1.0;
    }
}
