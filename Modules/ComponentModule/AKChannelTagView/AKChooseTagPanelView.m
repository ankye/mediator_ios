...
@implementation AKChooseTagPanelView
{
    AKChannelTagButton *_placeHolder;
}
...
- (void)addButtonWith:(AKNewsChannel *)channel  position:(CGPoint)originPoint
{
    ...
    AKChannelTagButton *button = [[AKChannelTagButton alloc] initWithFrame:(CGRect){originPoint, buttonSize} withChannel:channel];
...
//    self.backgroundColor = [UIColor blueColor];
    [UIView animateWithDuration:kDuration animations:^{
        ...
        for (int i  = 0; i < self.buttonArray.count; i++) {
            AKChannelTagButton *button = self.buttonArray[i];
            CGFloat buttonX = 0;
            CGFloat buttonY = 0;
            if (i == 0) {
                buttonX = kMarginW;
                ...
                AKChannelTagButton *foreButton = self.buttonArray[i - 1];
...
{
    _edit = edit;
    for (AKChannelTagButton *button in self.buttonArray) {
        button.edit = _edit;
    }
}

- (void)selectWithButton:(AKChannelTagButton *)button
{
...
{
    AKChannelTagButton *button = (AKChannelTagButton *)sender.view;
    switch (sender.state) {
        case UIGestureRecognizerStateBegan:
        {
            ...
            if (_placeHolder == nil) {
                _placeHolder = [[AKChannelTagButton alloc] initWithFrame:button.frame];
...
//            NSLog(@"xdelta is %f, ydelta is %f", Xdelta, Ydelta);
            ...
            if (newIndex < 0) {
                ...
                if (_placeHolder == nil) {
                    _placeHolder = [[AKChannelTagButton alloc] initWithFrame:(CGRect){CGPointZero, CGSizeMake(kButtonW, kButtonH)}];
                    [self.buttonArray removeObject:button];
                }else {
...
- (NSInteger)indexOfButtonWithMovingButton:(AKChannelTagButton *)movingButton
{
...
-(NSMutableArray*)getChannels
{
    ...
    for (AKChannelTagButton *button in self.buttonArray) {
        [channels addObject:button.channel];
    }
    return channels;
}
