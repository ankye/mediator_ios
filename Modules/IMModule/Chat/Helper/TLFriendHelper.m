...
- (id)init
{
    if (self = [super init]) {
        // 初始化好友数据
        self.friendsData = [AK_DB_MANAGER friendsDataByUid:[TLUserHelper sharedHelper].userID];
        ...
        // 初始化群数据
        self.groupsData = [AK_DB_MANAGER groupsDataByUid:[TLUserHelper sharedHelper].userID];
        // 初始化标签数据
...
- (void)p_initTestData
{
    ...
    // 更新好友数据到数据库
    BOOL ok = [AK_DB_MANAGER updateFriendsData:self.friendsData forUid:[TLUserHelper sharedHelper].userID];
    if (!ok) {
    ...
    ok = [AK_DB_MANAGER updateGroupsData:self.groupsData forUid:[TLUserHelper sharedHelper].userID];
    if (!ok) {
