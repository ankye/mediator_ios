...
- (instancetype)initWithFont:(CGFloat)font withLineSpace:(AKPagingLineSpaceType)lineSpace pageRect:(CGRect)pageRect
{
    ...
    if (self) {
        ...
        _pageOffsets = [[NSMutableArray alloc] init];
...
- (void)paginate:(NSString *)contentText {
    ...
    // 富文本
    NSMutableAttributedString *attrString = [[NSMutableAttributedString  alloc] initWithString:buffer attributes:strAttr];
    buffer                                = nil;// 马上释放
    ...
    while (hasMorePages) {
        ...
        if (samePlaceRepeatCount > 1) {
            ...
            if ([_pageOffsets count] == 0) {
                [_pageOffsets addObject:@(currentOffset)];
            } else {
                NSUInteger lastOffset = [[_pageOffsets lastObject] intValue];
        ...
        if ((range.location + range.length) != attrString.length) {
            ...
            [_pageOffsets removeLastObject];
            buffer                  = [self subStringWithRange:NSMakeRange(currentOffset, CHAR_PER_LOAD) contenText:_contentText];
            attrString              = [[NSMutableAttributedString alloc] initWithString:buffer attributes:strAttr];
           // [attrString setAttributes:strAttr range:NSMakeRange(0, attrString.length)];
            buffer                  = nil;
...
- (NSString *)stringOfPage:(NSUInteger)page {
    ...
    NSUInteger head = [_pageOffsets[page] intValue];
    NSUInteger tail = _contentText.length;
    if (page+1 < [self pageCount]) {
        tail = [_pageOffsets[page+1] intValue];
    }
...
- (NSDictionary *)stringAttrWithFont:(NSUInteger )fontSize {
    NSString* fontName = [AKReaderSetting sharedInstance].fontName;
    
    UIFont *font                            = [UIFont fontWithName:fontName size:fontSize];
    // 段的样式设置
    NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc] init];
    //行间距
    paragraphStyle.lineSpacing              =  _lineSpace*0.5 * font.pointSize / 3;
    
    paragraphStyle.paragraphSpacing         =  _lineSpace*0.5 * font.pointSize * 0.5;
    // 对齐
