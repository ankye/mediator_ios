...
- (id)parsingResponseObject:(id)response type:(YAPIType)type{
    ...
    if (type == YAPITypeBookDetail) {
        YBookDetailModel *bookD = [YBookDetailModel modelWithJSON:response];
        return bookD;
    }
    if (type == YAPITypeChapterContent) {
        if ([response[@"ok"] boolValue]) {
            YChapterContentModel *chapter = [YChapterContentModel modelWithJSON:response[@"chapter"]];
            return chapter;
        }
        return nil;
    }
        ...
        if ([response[@"ok"] boolValue]) {
            ...
            for (NSInteger i = 0; i < arr.count; i++) {
                for (NSDictionary *dict in arr[i]) {
                    YRankingModel *rankingM = [YRankingModel modelWithDictionary:dict];
                    if (rankingM) {
                        if (i == 0) {
    ...
    for (NSInteger i = 0 ; i < arr.count; i++) {
        ...
        if (type == YAPITypeFuzzySearch) {
            bookM = [YBookModel modelWithJSON:arr[i]];
        } else if (type == YAPITypeRecommendBook) {
            bookM = [YRecommendBookModel modelWithJSON:arr[i]];
        } else if (type == YAPITypeRecommendBookList) {
            bookM = [YRecommendBookListModel modelWithJSON:arr[i]];
        } else if (type == YAPITypeBookReview) {
            bookM = [YBookReviewModel modelWithJSON:arr[i]];
        } else if (type == YAPITypeBookSummary) {
            bookM = [YBookSummaryModel modelWithJSON:arr[i]];
        } else if (type == YAPITypeChaptersLink) {
            bookM = [YChaptersLinkModel modelWithJSON:arr[i]];
        } else if (type == YAPITypeBookUpdate) {
            bookM = [YBookUpdateModel modelWithJSON:arr[i]];
        } else if (type == YAPITypeRankingDetial) {
            bookM = [YBookModel modelWithJSON:arr[i]];//先这样
        }
